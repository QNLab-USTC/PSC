...
class RandomTopoQuantumRouting(Topology):
    ...
    def build(self) -> Tuple[List[QNode], List[QuantumChannel]]:
        ...
        for i in range(self.nodes_number - 1):
            ...
            link.capacity = 100
            link.capacity = np.random.binomial(link.capacity, p_out)
            tot_channel.append((idx + 1, i + 2, link.capacity))
        ...
        if self.lines_number > self.nodes_number - 1:
            for i in range(self.nodes_number - 1, self.lines_number):
                ...
                link.capacity = 100
                link.capacity = np.random.binomial(link.capacity, p_out)
                tot_channel.append((a + 1, b + 1, link.capacity))
                link.fidelity = self.init_fidelity
                link.request_ID_on_edge = []
                link.flow_on_edge = []
                link.traffic_on_edge = []
...
with open(file_path, mode='w', encoding='utf-8') as f:
    ...
    f.write('\n')
    for i in routing_net.qchannels:
    ...
    while cnt < request_num:
        # 产生request_num个固定跳数为request_length的请求
        # for src_idx in range(0, len(routing_net.nodes) + 1):
        #     if cnt == request_num:
        #         break
        #     for dst_idx in range(0, len(routing_net.nodes) + 1):
        #         if src_idx == dst_idx:
        #             continue
        ...
        #             continue
        #         if routing_net.query_route(routing_net.nodes[src_idx], routing_net.nodes[dst_idx])[0][
        #             2] != request_length:
        #             continue
        ...
        #         if cnt == request_num:
        #             break
        src_idx = get_randint(0, len(routing_net.nodes) - 1)
        dst_idx = get_randint(0, len(routing_net.nodes) - 1)
        new_request = Request(src=routing_net.nodes[src_idx], dest=routing_net.nodes[dst_idx])
        if src_idx == dst_idx or new_request in request:
            continue
        new_request.fidelity_threshold = random.uniform(0.7, 0.8)
        request_greedy.append(Request(src=greedy_net.nodes[src_idx], dest=greedy_net.nodes[dst_idx]))
        request.append(Request(src=routing_net.nodes[src_idx], dest=routing_net.nodes[dst_idx]))
...
with open("response_v1/link_failure_probability/result_0.05_Pu.txt", mode='a') as fff:
...
with open("response_v1/link_failure_probability/result_0.05_Greedy.txt", mode='a') as ffff:
